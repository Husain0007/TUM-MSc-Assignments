...
class NormalizeTransform:
    ...
    def __call__(self, images):
        ...
        if (len(images[0].shape)==2):
            images = (images - self.mean)*(1/self.std)
        elif(len(images[0].shape)==3):
            images[:,:,:,0] = (images[:,:,:,0] - self.mean[0])*(1/self.std[0])
            images[:,:,:,1] = (images[:,:,:,1] - self.mean[1])*(1/self.std[1])
            images[:,:,:,2] = (images[:,:,:,2] - self.mean[2])*(1/self.std[2])
