...
class ImageFolderDataset(Dataset):
    ...
    def __getitem__(self, index):
        #x=self.load_images_as_numpy(self.images[index])
        y=self.labels[index]
        
        if self.transform == None:
            x=np.array(Image.open(self.images[index]), dtype=np.float64)
        else:
            x=self.transform(np.asarray(Image.open(self.images[index]), dtype=np.float64))
